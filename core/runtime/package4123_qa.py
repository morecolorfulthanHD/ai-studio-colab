"""Package 4.12.3 consolidated automated QA (callable, reusable).

Extracted so production identity orchestration can invoke the same logic as
`core/scripts/qa_package4123.py` without duplicating suite selection or pass rules.
"""
...
from .studio_timezone import studio_date_stamp, studio_now
...
DEFAULT_PACKAGE4123_SUITES: tuple[str, ...] = (
...
def run_suite(repo_root: Path, script_name: str) -> dict[str, Any]:
...
def timezone_checks() -> dict[str, Any]:
    ...
    pdt_utc = datetime(2026, 7, 15, 19, 0, tzinfo=tz.utc)
    pdt_local = sn(pdt_utc)
    pst_utc = datetime(2026, 1, 15, 20, 0, tzinfo=tz.utc)
...
def run_consolidated_package4123_qa(
    repo_root: Path,
    *,
    report_path: Path | None = None,
    suites: tuple[str, ...] | list[str] | None = None,
    include_timezone_checks: bool = True,
    write_reports: bool = True,
    print_summary: bool = True,
) -> dict[str, Any]:
    """Authoritative Package 4.12.3 consolidated QA. Same pass rules as the CLI script."""
    ...
    target_report = report_path or default_report

    selected = list(suites) if suites is not None else list(DEFAULT_PACKAGE4123_SUITES)
    results = [run_suite(repo_root, name) for name in selected]
    tz_result = timezone_checks() if include_timezone_checks else {
        "name": "timezone_inline",
        "ok": True,
        "tests_discovered": 0,
        "passed": 0,
    ...
    all_ok = all(r["ok"] for r in results) and bool(tz_result.get("ok"))
    ...
    report: dict[str, Any] = {
        ...
        "consolidated_qa_status": "PASS" if all_ok else "FAIL",
        "ok": all_ok,
    }

    written_paths: list[str] = []
    if write_reports:
        for target in (target_report, repo_fallback):
            try:
                ...
                continue
    report["report_paths"] = written_paths
    report["report_path"] = written_paths[0] if written_paths else str(target_report)
