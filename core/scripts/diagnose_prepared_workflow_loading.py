#!/usr/bin/env python3
"""Read-only diagnostic for prepared workflow ComfyUI loading (Package 4.8.1)."""
...
from core.runtime.comfyui_userdata import DEFAULT_COMFY_BASE_URL, comfyui_reachable, userdata_get_workflow, userdata_list_workflows
...
def main() -> int:
    ...
    dest = comfyui_runtime / "user" / "default" / "workflows" / f"ai_studio_{preparation_id}.json"
    ...
    if reachable:
        ...
        listed = dest.name in names or any(dest.name in str(n) for n in names)
        got = userdata_get_workflow(base_url=args.comfy_url, filename=dest.name)
        if got.get("ok"):
            retrieved_match = bytes_sha256(got.get("body") or b"") == dest_sha if dest_exists else False
            if not dest_exists and got.get("ok"):
                retrieved_match = False
    ...
    recommended = "Run open_prepared_workflow.py while ComfyUI is running, then click the workflow name."
    if source_exists and dest_exists and reachable and retrieved_match:
        conclusion = "registered_and_verified"
        recommended = (
            f"In ComfyUI Workflows sidebar, left-click {dest.name} to open. "
        ...
        # Load copy may intentionally differ slightly (metadata stamps); compare node counts instead.
    ...
    payload = {
        ...
        "userdata_relative_path": f"workflows/{dest.name}",
        "source_exists": source_exists,
        "destination_exists": dest_exists,
        "source_sha256": source_sha,
        "destination_sha256": dest_sha,
        "hashes_match": bool(source_sha and dest_sha and source_sha == dest_sha),
