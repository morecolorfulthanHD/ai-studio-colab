#!/usr/bin/env python3
"""Compare a source generation to a variation preparation (Package 4.11)."""
...
from core.runtime.generation_derivation import compare_generation_to_derivation
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compare source generation to a variation preparation (read-only)."
    )
    parser.add_argument("--source-generation", required=True)
    parser.add_argument("--derivation-preparation", required=True)
    parser.add_argument("--child-generation", default="", help="Optional child generation ID after Run.")
    ...
    try:
        ...
        return 1
    try:
        preparation_id = normalize_preparation_id(args.derivation_preparation)
    ...
    gen_manifest = _load_json(snapshot_root / MANIFEST_FILENAME)
    ...
    child_metadata = None
    if args.child_generation:
        try:
            child_id = normalize_generation_id(args.child_generation)
        except InvalidGenerationIdError as exc:
            print(str(exc), file=sys.stderr)
            return 1
        child_manifest = load_snapshot_by_id(drive_root, child_id)
        if child_manifest is None:
            print(f"ERROR: Child generation not found:\n{child_id}", file=sys.stderr)
            return 1
        child_root = Path(str(child_manifest.get("snapshot_root") or ""))
        child_metadata = _load_json(child_root / METADATA_FILENAME)

    report = compare_generation_to_derivation(
        generation_metadata=gen_metadata,
        generation_manifest=gen_manifest,
        preparation_metadata=prep_metadata,
        child_generation_metadata=child_metadata,
        prepared_dir=prepared_dir if prepared_dir.is_dir() else None,
        generation_workflow=gen_workflow,
    )
    report["source_generation_id"] = generation_id
    report["derivation_preparation_id"] = preparation_id
    if args.child_generation:
        report["child_generation_id"] = normalize_generation_id(args.child_generation)

    if args.json:
        print(json.dumps(report, indent=2))
        return 0 if report.get("all_match") else 1

    print("AI Studio — Generation / Derivation Comparison")
    print("=" * 40)
    print(f"Source generation:           {generation_id}")
    print(f"Variation preparation:       {preparation_id}")
    print(f"Lineage OK:                  {'yes' if report.get('lineage_ok') else 'no'}")
    print(f"All checks match:            {'yes' if report.get('all_match') else 'no'}")
    if report.get("child_lineage_ok") is not None:
        print(f"Child lineage OK:            {'yes' if report.get('child_lineage_ok') else 'no'}")
    print()
