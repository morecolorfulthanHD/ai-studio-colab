...
def main() -> int:
    ...
    zip_path = _collision_safe_path(out_dir / f"{args.generation_id}.zip")
    ...
    payload = {"generation_id": args.generation_id, "export_path": str(zip_path), "files": len(file_hashes) + 1}
    if args.json:
        print(json.dumps(payload, indent=2))
    else:
        print(f"Exported generation: {args.generation_id}")
