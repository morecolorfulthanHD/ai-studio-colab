#!/usr/bin/env python3
"""Prepare a temporary Qwen-Image-Edit-2511 benchmark workflow."""
...
QWEN_REQUIRED = [
    "qwen_image_edit_2511_diffusion",
    "qwen_image_edit_text_encoder",
    "qwen_image_vae",
]
QWEN_REFERENCE = "workflows/reference/qwen_image_edit/workflow.json"


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Prepare temporary Qwen Image Edit 2511 benchmark workflow (no auto-download)."
    )
    parser.add_argument("--input", type=Path, required=True)
    parser.add_argument("--reference-image", type=Path, default=None, help="Optional second reference image.")
    ...
    result = prepare_modern_editing_workflow(
        ...
        reference_relpath=QWEN_REFERENCE,
        required_models=QWEN_REQUIRED,
    ...
    if args.json:
        ...
        print("\nAI Studio — Qwen Image Edit Preparation")
        print("=" * 40)
    ...
    if not result.ok:
        print("\nRESULT: FAIL — Qwen preparation reported errors.", file=sys.stderr)
        return 1
    print("\nRESULT: OK — Qwen preparation complete.")
    return 0
